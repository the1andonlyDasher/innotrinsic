...
export function Model(props: NeuronsProps) {
  ...
  const { nodes, materials, animations, userData, scene }: any = useGLTF('/neuron4.glb') as GLTFResult
  ...
  return (
    <group visible={!disposed} ref={group}{...props} dispose={null}>
      <group name="Scene">
        <Float position={props.location} rotation={[0.735, 1.235, 0.5]} scale={size(0.8, viewport.width / 10, 2)} visible={!disposed} rotationIntensity={0.2} floatIntensity={0.2} floatingRange={[-0.2, 0.2]}>
...
export const Neuron = ({ scroll }: any) => {
  const { viewport } = useThree();
  return (<>
    <Model scroll={scroll} location={[-viewport.width / 2 + 3, viewport.height / 2 - 2, 0]} />
    <Model scroll={scroll} location={[viewport.width / 2 + 3, -viewport.height / 2 - 2, 0]} />
    <Model scroll={scroll} location={[viewport.width / 2 + 3, viewport.height / 2 - 2, 0]} />
    <Model scroll={scroll} location={[-viewport.width / 2 + 3, -viewport.height / 2 - 2, 0]} />
  </>)
