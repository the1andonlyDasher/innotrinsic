...
export function TexturedHand(props: JSX.IntrinsicElements['group']) {
  ...
  const { nodes, materials, animations } = useGLTF('/texturedHand2.glb') as GLTFResult
  ...
  React.useEffect(() => {
    router.pathname === "/einsatzgebiete" ?
      setDisposed(false) :
      setTimeout(() => { setDisposed(true) }, 200)
  }, [router.pathname])
  ...
  useFrame(() => {
    if (materials && materials.Arms__Standard_ && !disposed) {
      materials.Arms__Standard_.opacity = lerp(materials.Arms__Standard_.opacity, router.pathname === "/" ? 0 : 1, 0.15)
      materials.Arms__Standard_.needsUpdate = true
    }
  })

  return (
    <group visible={!disposed} ref={group} scale={1} position={[0.6, 0.6, 0.15]} rotation={[Math.PI / -1.0, 1.9, -0.3]} {...props} dispose={null}>
      <mesh
        ref={mesh}
